...
public class LinkedProviderBindingImplSapientGeneratedJunit4Test {
    ...
    //Sapient generated method id: ${903fef97-987f-323e-a528-d02591a6bcb2}, hash: 2A8E337BE2EEA77E89AB0A2F032012EE
    @Test()
    public void getDependenciesTest() {
        //Arrange Statement(s)
        Dependency dependencyMock = mock(Dependency.class, "Dependency");
        try (MockedStatic<Dependency> dependency = mockStatic(Dependency.class)) {
            dependency.when(() -> Dependency.get((Key) any())).thenReturn(dependencyMock);
            ...
            ImmutableSet<Dependency<?>> anySet = ImmutableSet.of(dependencyMock);
            //Assert statement(s)
            assertThat(result, equalTo(anySet));
            dependency.verify(() -> Dependency.get((Key) any()));
        }
    }
