...
public class MissingConstructorErrorSapientGeneratedJunit4Test {
    ...
    //Sapient generated method id: ${f9cf5d02-0fb8-3666-9293-4ef04ec8dde2}, hash: 7B32A0704195CF9E763F2CE4B6D9CDE2
    @Ignore()
    @Test()
    public void formatDetailWhenSourcesListIsNotEmpty() throws NoSuchMethodException {
        ...
        try (MockedStatic<ErrorFormatter> errorFormatter = mockStatic(ErrorFormatter.class);
            ...
            Formatter formatter = formatterMock.format("\n", objectArray);
            doReturn(formatter).when(formatterMock).format("\n", objectArray);
            Object[] objectArray2 = new Object[]{Object.class};
            Formatter formatter2 = formatterMock.format("Injector is configured to require @Inject constructors but %s does not have a @Inject annotated constructor.\n", objectArray2);
            doReturn(formatter2).when(formatterMock).format("Injector is configured to require @Inject constructors but %s does not have a @Inject annotated constructor.\n", objectArray2);
            Object[] objectArray3 = new Object[]{};
            Formatter formatter3 = formatterMock.format("\n", objectArray3);
            doReturn(formatter3).when(formatterMock).format("\n", objectArray3);
            String[] stringArray = new String[]{"String"};
            Formatter formatter4 = formatterMock.format("%s\n", stringArray);
            doReturn(formatter4).when(formatterMock).format("%s\n", stringArray);
            ...
            doReturn(Object.class).when(typeMock).getRawType();
